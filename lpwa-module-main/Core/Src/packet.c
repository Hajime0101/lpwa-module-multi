...
// パケットからメッセージ本体のポインタを取得する
static uint8_t *get_msg_from_packet(uint8_t *packet)
{
	uint8_t *msg = packet;
	switch( *packet ){
	case 0x01:
...
static void packet_initialize(uint8_t packet[], int packet_size){
	...
	uint8_t my_area = RetrieveArea(my_id);
	// HQであれば何もしない
	if( IsHQ(my_id) ) return;
	// areaをチェックする
...
static void packet_downlink(enum PACKET_CMD_TYPE cmd, uint8_t packet[], int packet_size){
	...
	if( flag_repeat ){
		int i;
		// ConfigReadTxRepeat() が 0 の場合は中継しない
	...
	if( flag_store ){
		// check `!`
		uint8_t *msg = get_msg_from_packet(packet);
		if( *msg == '!' ){
			ConrtolCommandExecute(msg);
		} else {
