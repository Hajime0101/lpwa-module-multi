...
static int command_info(void){
    ...
    ConfigReadVersion(product);
    product[8] = 0;
    // 文字列作成
    SerialOutput("Product: %s\r\n", product);
    SerialOutput("Version: "MEMORY_VERSION_STR"\r\n");
    SerialOutput("DeviceID: %02X %04X %s\r\n", RetrieveArea(my_id), RetrieveDevice(my_id),
    		IsHQ(my_id)?"(HQ)":"");
    SerialOutput("UplinkID: %02X %04X %s\r\n", RetrieveArea(uplink_id), RetrieveDevice(uplink_id),
    		IsHQ(uplink_id)?"(HQ)":"");
    SerialOutput("Seq: %d\r\n", ConfigReadSeq());
    SerialOutput("Tx Repeat: %d\r\n", ConfigReadTxRepeat());
	return 0;
}
...
static int command_settings(const uint8_t *param){
	...
	} else	if( check_substr(param, "TX") ){
		// TX Repeat
		int t;
		if( isSetValue ){
			sscanf(value, "%d", &t);
			ConfigWriteTxRepeat(t);
			ret = 1;
		} else {
			t = ConfigReadTxRepeat();
			SerialOutput("%d\r\n", t);
			ret = 0;
		}
...
static int command_function(const uint8_t *param){
	uint8_t func_no = Hex1toNum1(param[0]) * 16 + Hex1toNum1(param[1]);
	int isSetValue = ( param[2] == '=' );
	if( isSetValue ){
		// ATF<func_no>=<value>, 値を設定する
		uint8_t value = Hex1toNum1(param[3]);
		if( param[4] != 0 ){
			value = value * 16 + Hex1toNum1(param[4]);
		}
		ConfigWriteFeature(func_no, value);
		return 1;
	} else {
		// ATF<func_no>, 値を表示する
		uint8_t value = ConfigReadFeature(func_no);
		SerialOutput("F%02x: %02x\r\n", func_no, value);
		return 0;
	}
}
...
static int command_broadcast(const uint8_t *param){
	...
	// TX
	int i;
	for( i=0 ; i<ConfigReadTxRepeat() ; i++ ){
...
static int command_time(const uint8_t *param){
	SerialOutput("TIME: %d\r\n", tc);
    return 0;
}
...
void CommandExecute(const uint8_t *cmd) {
	...
	} else if (check_substr(cmd, "AT/")) {  // SETTINGS
	...
	//add
	} else if (check_substr(cmd, "ATT")) {  // TIME
		ret = command_time(cmd);
	//add
